...
- **Preview must be a CONTIGUOUS run, not a stretched single frame** (`handleScrubPreview`): a paused `<video>` paints a seek into a contiguous multi-frame region (this is why scrubbing the already-buffered area always worked) but will NOT settle on a lone I-frame stretched over a gap — it sits at `HAVE_METADATA` and never paints, so previews only appeared on release. So the scrub preview now fetches `gop + ~0.5 s lookahead` REAL consecutive frames (capped ~1.5 s to bound MPEG-2 decode), `stretchToFrames: 0`, and the player seeks the dragged target inside that run. This is what made frames paint *during* the drag (verified: `getVideoPlaybackQuality().totalVideoFrames` rises mid-scrub, was 0 before). **Remaining limit:** each cycle is still a fetch→decode→append→seek round-trip, so a *very* fast drag (faster than that round-trip) outruns it and paints fewer intermediate frames — but normal/slow scrubbing now paints live, and it recovers on release. (A canvas/WebCodecs surface would remove even the round-trip latency, but isn't needed now.)
