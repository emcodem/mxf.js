...
- **`MxfFile.locateEssence`**: KLV-walks the essence-bearing partition (chosen via RIP `bodySID>0` entry), collecting index segments, stopping at the first Generic Container element (`isGenericContainerElement`: key bytes [8..11] = `0D 01 03 01`). Byte-count math is not trusted. Uses `readKLVHeader` + re-probe (not a fixed window) so it handles KAG-aligned files where the essence starts exactly at or beyond 1 MB — XAVC-L from Atomos Shogun Max uses KAG=512 KB, placing index at 512 KB and essence at 1 MB (exactly the old `ESSENCE_SCAN_WINDOW`). `collectMultiPartitionIndex` skips RIP entries with `byteOffset=0`; when the body partition IS the header (`bodySID=2` at offset 0), `essenceStart` from `locateEssence` seeds the remapping.
