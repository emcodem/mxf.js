...
import { ulMatchClass, formatUL, bytesEqual, isPartitionPack, isIndexTableSegment, isGenericContainerElement } from '../core/ul.js';
...
// Byte 6 (1-indexed, value 0x7f) is a wildcard — ulMatchClass skips bytes 5-7.
// Format: 06 0E 2B 34 02 7F 01 01 | 0D 01 01 01 01 01 XX 00
function cls(...b: number[]): Uint8Array { return new Uint8Array(b); }

const CLASS_MATERIAL_PACKAGE = cls(0x06,0x0e,0x2b,0x34,0x02,0x7f,0x01,0x01,0x0d,0x01,0x01,0x01,0x01,0x01,0x36,0x00);
const CLASS_FILE_PACKAGE     = cls(0x06,0x0e,0x2b,0x34,0x02,0x7f,0x01,0x01,0x0d,0x01,0x01,0x01,0x01,0x01,0x37,0x00);
const CLASS_SOURCE_PACKAGE   = cls(0x06,0x0e,0x2b,0x34,0x02,0x7f,0x01,0x01,0x0d,0x01,0x01,0x01,0x01,0x01,0x38,0x00);
const CLASS_STATIC_TRACK     = cls(0x06,0x0e,0x2b,0x34,0x02,0x7f,0x01,0x01,0x0d,0x01,0x01,0x01,0x01,0x01,0x3a,0x00);
const CLASS_TRACK            = cls(0x06,0x0e,0x2b,0x34,0x02,0x7f,0x01,0x01,0x0d,0x01,0x01,0x01,0x01,0x01,0x3b,0x00);
const CLASS_SEQUENCE         = cls(0x06,0x0e,0x2b,0x34,0x02,0x7f,0x01,0x01,0x0d,0x01,0x01,0x01,0x01,0x01,0x0f,0x00);
const CLASS_TIMECODE_COMPONENT = cls(0x06,0x0e,0x2b,0x34,0x02,0x7f,0x01,0x01,0x0d,0x01,0x01,0x01,0x01,0x01,0x14,0x00);
// Descriptor sets
const CLASS_CDCI_DESCRIPTOR     = cls(0x06,0x0e,0x2b,0x34,0x02,0x7f,0x01,0x01,0x0d,0x01,0x01,0x01,0x01,0x01,0x28,0x00);
const CLASS_RGBA_DESCRIPTOR     = cls(0x06,0x0e,0x2b,0x34,0x02,0x7f,0x01,0x01,0x0d,0x01,0x01,0x01,0x01,0x01,0x29,0x00);
const CLASS_MPEGVID_DESCRIPTOR  = cls(0x06,0x0e,0x2b,0x34,0x02,0x7f,0x01,0x01,0x0d,0x01,0x01,0x01,0x01,0x01,0x51,0x00);
const CLASS_AVC_DESCRIPTOR      = cls(0x06,0x0e,0x2b,0x34,0x02,0x7f,0x01,0x01,0x0d,0x01,0x01,0x01,0x01,0x01,0x5a,0x00);
...
export function parseHeaderMetadata(
  ...
  while (iter.offset < end && iter.hasMore()) {
    ...
    // Stop at a body/footer partition pack or generic container essence — they mark the end of
    // this partition's metadata region. Index Table Segments (0D 01 02 01 / byte10 === 0x02) are
    // skipped rather than breaking: some encoders write the index segment BEFORE the essence
    // descriptor in the header partition (→ descriptor not reached → pictureDescriptor=null).
    // Descriptive Metadata sets (0D 01 04 01) are valid header metadata and are passed through.
    if (isGenericContainerElement(k) || isPartitionPack(k)) break;
    if (isIndexTableSegment(k)) continue;
