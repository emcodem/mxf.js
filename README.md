...
| `maxSourceCacheBytes` | `number` | `268435456` (256 MB) | RAM the worker may use to cache raw, compressed **source** bytes already fetched, so backward seeks, loops, and scrub-revisits don't re-download. A whole moderate file often fits, making re-seeks network-free. This is the compressed source — distinct from (and far smaller than) the decoded video MSE holds. `0` disables it. Ignored in live mode (growing recordings are never cached). |
