...
const C = 1, P = 0.5, A = 3, B = 6, D = C + 0.5;
class b extends R {
  constructor(e, t = !1) {
...
const L = 40, N = 0.25, $ = 0.08, U = 2, I = 30, m = class m {
  ...
  makeSource(e) {
    ...
    const d = [], o = [];
    n.forEach((f, p) => (p % 2 === 0 ? d : o).push(f)), n.length === 1 && (o.length = 0, o.push(n[0]));
    const c = t.createBuffer(2, a, r), l = (f, p) => {
      if (p.length !== 0)
        for (let v = 0; v < a; v++) {
          let k = 0;
          const T = v * s;
          for (const w of p) k += i[T + w];
          f[v] = k;
        }
    };
  ...
  evict(e) {
    ...
    for (const i of this.store) {
      if (i.mediaEnd < e - U || i.mediaStart > e + I) {
        if (i.source)
          try {
...
class V extends R {
  ...
  systemTimecodeAt(e) {
    ...
    return M(F(i, t.base, t.dropFrame));
  }
  ...
  computeTimecodeBundle(e) {
    ...
    for (const o of this.manifestTimecodes) {
      const c = o.editRateDenominator > 0 ? o.editRateNumerator / o.editRateDenominator : r, l = r > 0 ? Math.round(e * (c / r)) : e, u = M(F(o.position + l, o.base, o.dropFrame));
  ...
  loadUrl(e) {
    var s;
    this.setup();
    const t = (s = this.config.plugins) != null && s.videoDecoder ? { videoDecoder: g(this.config.plugins.videoDecoder) } : void 0, i = { type: "initUrl", url: e, debug: this.config.debug, videoMode: "mse", plugins: t };
  ...
  loadLive(e) {
    var s;
    this.setup();
    const t = (s = this.config.plugins) != null && s.videoDecoder ? { videoDecoder: g(this.config.plugins.videoDecoder) } : void 0, i = { type: "initUrl", url: e, debug: this.config.debug, videoMode: "mse", plugins: t, live: !0, startEditUnit: 0, liveFromStart: !1 };
    this.worker.postMessage(i);
  }
  loadFile(e) {
    var s;
    this.setup();
    const t = (s = this.config.plugins) != null && s.videoDecoder ? { videoDecoder: g(this.config.plugins.videoDecoder) } : void 0, i = { type: "initFile", file: e, debug: this.config.debug, videoMode: "mse", plugins: t };
    this.worker.postMessage(i);
  }
  ...
  async onWorkerMessage(e) {
    ...
    switch (e.type) {
      ...
      case "liveTailFlushed":
        this.switching && (this.nextFetchFrame = e.nextEditUnit, this.config.liveReuseHeader ? this.continueLiveToNext() : this.activateStandby());
        break;
      case "liveContinued":
        this.onLiveContinued(e);
        break;
  ...
  async onManifest(e) {
    ...
    this.liveMode && (e.essenceStart !== void 0 && (this.cachedEssenceStart = e.essenceStart), (o = e.firstEssenceKey) != null && o.length && (this.cachedEssenceKey = e.firstEssenceKey)), this.editRateNumerator = e.editRateNumerator, this.editRateDenominator = e.editRateDenominator, this.audio.setEditRate(e.editRateNumerator, e.editRateDenominator), this.scrub.setStream(e.duration, e.editRateNumerator, e.editRateDenominator);
    const r = e.editRateNumerator / e.editRateDenominator;
    this.framesPerChunk = Math.ceil(r * C), this.rampChunkFrames = Math.max(A, Math.ceil(r * P)), this.startupGating = !0, this.manifestTimecodes = e.timecodes ?? [], this.systemAnchors = [], this.lastTimecodeEditUnit = -1, this.currentTimecodeBundle = null, this.manifest = {
      duration: s,
   ...
   */
  maybeCompleteLive() {
    return (this.config.liveReuseHeader ? this.reuseNextArmed : this.standbyReady) && this.liveStallPolls >= 1 ? (this.config.liveReuseHeader ? this.beginReuseSwitch() : this.beginGaplessSwitch(), !0) : (this.liveStallPolls < this.LIVE_STALL_MAX || this.liveEndEmitted || (this.liveEndEmitted = !0, this.emit("live-end", void 0)), !1);
  }
  /**
  ...
  preloadNextUrl(e) {
    ...
    const i = (s = this.config.plugins) != null && s.videoDecoder ? { videoDecoder: g(this.config.plugins.videoDecoder) } : void 0;
    t.postMessage({
      type: "initUrl",
      url: e,
  ...
  switchLive(e) {
    ...
    if (this.config.liveReuseHeader) {
      this.pendingNextUrl = e, this.reuseNextArmed = !0, this.liveAtEdge && this.liveStallPolls >= 1 && this.beginReuseSwitch();
      return;
    }
  ...
  activateStandby() {
    ...
    if (this.pendingNextUrl = null, this.log(`live: gapless switch to next file at editUnit ${this.nextFetchFrame}`), this.config.debug) {
      const r = this.video.buffered;
      let a = 0;
      for (let o = 0; o < r.length; o++) a = Math.max(a, r.end(o));
      const n = this.editRateNumerator ? this.nextFetchFrame * this.editRateDenominator / this.editRateNumerator : 0, d = this.video.currentTime;
      console.log(`[mxfdiag] SEAM base=${this.nextFetchFrame} seamSec=${n.toFixed(3)} bufEnd=${a.toFixed(3)} cur=${d.toFixed(3)} ahead=${(a - d).toFixed(3)}s overlap=${(a - n).toFixed(3)}s ranges=${this.videoRanges()}`);
    }
    i && this.emit("live-switched", { url: i }), this.fetchNextChunk(), this.scheduleLivePoll();
  }
  /**
  ...
  beginReuseSwitch() {
    var e;
    this.switching || this.pendingNextUrl && (this.switching = !0, this.livePollTimer !== null && (clearTimeout(this.livePollTimer), this.livePollTimer = null), this.seqBase += 2, (e = this.worker) == null || e.postMessage({ type: "flushLiveTail", seqBase: this.seqBase }));
  }
  /**
   ...
   */
  onLiveContinued(e) {
    this.nextFetchFrame = e.nextEditUnit, this.reuseNextArmed = !1, this.liveAtEdge = !1, this.liveStallPolls = 0, this.liveEndEmitted = !1, this.fetchPending = !1, this.previewParked = !1, this.bufferFull = !1, this.switching = !1;
    const t = this.pendingNextUrl;
    if (this.pendingNextUrl = null, this.log(`live: header-reuse continuation at editUnit ${this.nextFetchFrame} (reusedHeader=${e.reusedHeader})`), this.config.debug) {
      const i = this.video.buffered;
      let s = 0;
      for (let n = 0; n < i.length; n++) s = Math.max(s, i.end(n));
      const r = this.editRateNumerator ? this.nextFetchFrame * this.editRateDenominator / this.editRateNumerator : 0, a = this.video.currentTime;
      console.log(`[mxfdiag] SEAM(reuse) base=${this.nextFetchFrame} seamSec=${r.toFixed(3)} bufEnd=${s.toFixed(3)} cur=${a.toFixed(3)} ahead=${(s - a).toFixed(3)}s overlap=${(s - r).toFixed(3)}s reused=${e.reusedHeader} ranges=${this.videoRanges()}`);
    }
    t && this.emit("live-switched", { url: t }), this.fetchNextChunk(), this.scheduleLivePoll();
  }
  /**
